...
static bool load_from_shell_path(REFKNOWNFOLDERID rfid, const wchar_t *subpath)
{
	...
	CA_LOG(LOG_INFO, "Looking for 'CoreAudioToolbox.dll' in a directory '%ls'...", path);
	...
	if (!audio_toolbox) {
		...
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS |
			      ...
			      nullptr, err, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), (LPSTR)&msg, 0, nullptr);
		CA_LOG(LOG_INFO, "LoadLibrary failed: %s", msg);
		if (msg)
			LocalFree(msg);
	}
